...
it("distinguishes queued work from generating content without invented percentages", () => {
  const queued = renderToStaticMarkup(
    createElement(GenerationProgress, { pkg }),
  );
...
it("reports completion only when both content and narration are ready", () => {
  expect(
    renderToStaticMarkup(
      createElement(GenerationProgress, {
        pkg: { ...pkg, status: "ready", speech: "ready" },
      }),
    ),
  ).toContain("內容與語音都已準備完成");
  expect(
    renderToStaticMarkup(createElement(GenerationProgress, { planning: true })),
  ).toContain("正在根據你的回答");
});
