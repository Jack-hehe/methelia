...
it.each(["environment", "pinned"])("plays prepared narration with a %s voice profile and no synthesis key", (source) => {
  ...
  const profile: SpeechProfile = {provider:"elevenlabs",voiceId:"cached-voice",model:"eleven_flash_v2_5",languageCode:"en"};
  ...
  storeFeaturedAudio(store, featuredAudioKey(chapter.script, profile), {audio,cues,captions:[]});
  const snapshot = service.createFeatured(service.session(), "equation-explorer", "en");
...
it("does not schedule synthesis on a cache miss without an API key", () => {
  ...
  stores.push(store);
  const service = new LearningService(store);
  const course = service.createFeatured(service.session(), "equation-explorer", "en");
  expect(course.chapters[course.currentNodeId].speech).toBe("failed");
  expect(course.chapters[course.currentNodeId].error).toContain("ELEVENLABS_API_KEY");
  expect(store.db.prepare("SELECT count(*) AS n FROM generation_jobs WHERE kind='speech'").get()).toEqual({n:0});
});
...
it("retries previously failed narration from a newly available cache without a synthesis key",()=>{
  ...
  const store = new Store(":memory:");
  stores.push(store);
  const service = new LearningService(store),session=service.session();
  ...
  const profile:SpeechProfile={provider:"elevenlabs",voiceId:"retry-cached-voice",model:"eleven_flash_v2_5",languageCode:"en"};
  storeFeaturedAudio(store,featuredAudioKey(pkg.chapter!.script,profile),{audio:new Uint8Array([73,68,51]),cues:[],captions:[]});
  expect(service.retry(session,course.id,course.currentNodeId).chapters[course.currentNodeId].speech).toBe("ready");
  expect(store.db.prepare("SELECT count(*) AS n FROM generation_jobs WHERE kind='speech'").get()).toEqual({n:0});
});
