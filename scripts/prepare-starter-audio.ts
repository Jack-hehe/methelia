...
import { resolveSpeechProfile, synthesize } from "../src/server/speech";
...
const directory = resolve("public/starter-audio");
const entries = [];
for (const course of starterCourses.filter(
  ...
  for (const language of ["zh-TW", "en"] as const) {
    ...
    const nodes = demoGraph(language).nodes;
    for (const node of firstOnly ? nodes.slice(0, 1) : nodes) {
      const chapter = demoChapter(node, language);
      const reviewed = readFileSync(resolve(`docs/course-scripts/starter-website-${language}.md`), "utf8");
      if (chapter.script.some(segment => !reviewed.split(/\r?\n/).includes(segment.text)))
        throw new Error(`Review the updated ${language}/${node.id} script before preparing audio.`);
