...
try {
  for (const entry of featuredCourses) {
    for (const language of ["en", "zh-TW"] as const) {
      ...
      for (const node of featuredGraph(entry.id, language).nodes) {
        const chapter = featuredChapter(entry.id, node, language);
        const key = featuredAudioKey(chapter.script, profile);
        const path = resolve("public/featured-audio", key);
