...
export function reserveUsage(
  ...
  const unlimited = configured?.toLowerCase() === "unlimited";
  const limit = Number(configured || (kind === "ai" ? 100 : 30000));
  if (!unlimited && (!Number.isSafeInteger(limit) || limit < 0))
    throw new UsageLimitError(`${variable} 設定必須是非負整數或 unlimited。`);
  ...
  if (!accepted)
    throw new UsageLimitError(
      `今日${kind === "ai" ? "AI 請求" : "語音字數"}已達設定上限；仍可使用已準備的課程。請管理者調整額度或隔日重試（UTC）。`,
    );
}
