...
export const learningCapabilities = {
  ...
  laboratoryModels: {
    geometry:
      "Rotatable wireframe sphere x²+y²+z²=radius² and horizontal y=slice circular section; slice radius sqrt(radius²-slice²). Only this family, not an arbitrary equation parser.",
    calculus:
      "Coaster curve y=bend*x²/3 + (x>=0 ? join*x : 0). Move x to inspect tangent; join=0 removes slope discontinuity at zero.",
    probability:
      "Seeded independent trials. Player expected profit per play = probability*reward-cost. trials and seed change simulated sample, not theoretical expectation.",
    collision:
      "One-dimensional, isolated two-mass collision with restitution0..1; velocity1 nonnegative and velocity2 nonpositive. Compare momentum and kinetic energy.",
    orbit:
      "Normalized central gravity mu=1, planet radius0.5. speed is MULTIPLIER of circular velocity:1 is circular, sqrt2 is escape; actual initial velocity=speed/sqrt(radius). Numerical trajectory, no atmosphere.",
    circuit:
      "Ideal resistive load voltage/resistance; parallel=1 adds identical resistor in parallel. Bulb brightness indicates total dissipated power, not an LED model.",
    sound:
      "Oscillator frequency, amplitude, sine/square/saw wave and attack/release. Four saved semitone melody notes, explicit play/stop. No overlapping-note sequencer.",
    color:
      "Additive RGB screen-light, black/white text contrast and CSS rgb() export; no paint mixing.",
    design:
      "Preset science-fair poster, adjustable headline size, spacing, hue, columns and aspect ratio; SVG export. Not freeform text editing.",
    animation:
      "Bouncing character with duration, height, squash and linear/parabolic easing; timeline and SVG export.",
    ecosystem:
      "Logistic prey growth and predator interaction, fixed Euler step .1 for160 steps. Educational model, not real ecology prediction.",
    separation:
      "Fixed mixture 8 sand,12 salt,40 water; filter removes sand and evaporation leaves salt. No arbitrary molecular reaction engine.",
    flood:
      "Rain depth, percent green and storage depth. Infiltration=min(rain,30)*green/100; runoff=rain-infiltration-stored. Simplified water balance.",
    economy:
      "Coffee demand=round(traffic*(1-.16*(price-5))); sales=min(stock,demand); profit deducts ALL prepared cups plus fixed cost. Toy scenario, no live financial data.",
    logic:
      "Door opens for (gate0 ? A AND B : A OR B) AND NOT C; C is alarm. Toggle truth-table inputs.",
    pathfinding:
      "8x8 editable wall grid, BFS shortest four-neighbor path; fixed start0 and end63, maze presets. Not an arbitrary code editor.",
