...
不需要 API key 也能試用：點選「先體驗一堂課」直接進入課程。首頁也可返回目前課程；`/explore` 提供作品與路徑構想展示，**不是已開放的課程目錄或真實學員作品庫**。

左上角 Methelia 固定回到「I want to learn…」首頁，重新整理也不會自動跳回課程；按「我的課程」才恢復原本的學習進度。
